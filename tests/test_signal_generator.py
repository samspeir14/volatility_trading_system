...
def test_constructor_requires_exactly_one_prediction_source():
    """The generator runs either the h=1 deviation predictor (active) or the
    legacy multi-horizon predictors — never both, never neither."""
    predictors = {h: _ConstPredictor() for h in (5, 10, 21)}

    class _FakeH1:
        def predict_deviation(self, X_row):
            return 0.0
    ...
    try:
        ...
        raise AssertionError("no prediction source should raise")

    try:
        SignalGenerator(h1_predictor=_FakeH1(), predictors_by_horizon=predictors)
    except ValueError as e:
        assert "exactly one" in str(e)
        print("constructor: exactly-one-source contract enforced")
    else:
        raise AssertionError("both prediction sources should raise")
